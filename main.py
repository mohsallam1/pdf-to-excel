...
# Try to import scipy, make it optional if not available
try:
...
class PDFTableExtractor:
    ...
    def extract_raw_text_vectors(self) -> pd.DataFrame:
        ...
        if not PDFPLUMBER_AVAILABLE or pdfplumber is None:
            self.log("⚠ pdfplumber not available, skipping raw text extraction", "WARNING")
            return pd.DataFrame()
    ...
    def extract_camelot_lattice(self) -> pd.DataFrame:
        ...
        if not CAMELOT_AVAILABLE or camelot is None:
            self.log("⚠ Camelot not available, skipping Camelot extraction", "WARNING")
            return pd.DataFrame()
    ...
    def extract_ocr_cells(self, page_num: int = 1, timeout_seconds: int = 30) -> Dict[Tuple[int, int], str]:
        ...
        try:
            ...
            if not PYMUPDF_AVAILABLE or fitz is None:
                self.log("⚠ PyMuPDF not available, skipping OCR image processing", "WARNING")
                return {}
        ...
        for col, dtype in column_types.items():
            if dtype in ['currency', 'integer'] and col in df.columns:
                try:
                    ...
                    # Z-score method (if scipy is available)
                    if SCIPY_AVAILABLE and stats:
                        z_scores = np.abs(stats.zscore(numeric_col.dropna()))
                    else:
                        z_scores = pd.Series([0] * len(numeric_col.dropna()), index=numeric_col.dropna().index)
